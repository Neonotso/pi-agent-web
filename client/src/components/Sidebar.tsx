...
export default function Sidebar({ onClose }: SidebarProps) {
  const { sessions, activeSessionId, createSession, switchSession, deleteSession, renameSession } = usePiAgent();
  ...
  return (
    <>
      ...
      <aside className="fixed lg:relative inset-y-0 left-0 z-50 w-72 bg-surface-900 border-r border-surface-800 flex flex-col">
        ...
        {/* New Chat */}
        <div className="p-3">
          <button onClick={() => { createSession(); onClose(); }} className="w-full btn btn-primary justify-center">
        ...
        <div className="flex-1 overflow-y-auto px-2 pb-2">
          <div className="text-[10px] uppercase tracking-wider text-slate-600 font-semibold px-3 py-2">Chats</div>
          {sessions.length === 0 && (
          ...
          {sessions.map((session) => (
            ...
            >
              ...
              <div className="sidebar-delete absolute right-1 top-1/2 -translate-y-1/2 flex items-center gap-0.5 opacity-0 transition-opacity">
                <button onClick={(e) => { e.stopPropagation(); startRename(session); }} className="p-0.5 rounded hover:bg-surface-700 text-slate-400 hover:text-white">
                  <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                  </svg>
                </button>
                <button onClick={(e) => handleDelete(e, session.id)} className="p-0.5 rounded hover:bg-red-900/30 text-slate-400 hover:text-red-400">
                  <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="p-3 border-t border-surface-800 text-[10px] text-slate-600 text-center">
          π Agent v0.2.0 · {sessions.length} session{sessions.length !== 1 ? 's' : ''}
        </div>
      </aside>
    </>
  );
