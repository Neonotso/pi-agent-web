    ...
    <div className="flex flex-col h-full">
      ...
      <header className="flex items-center justify-between px-4 py-3 border-b border-surface-800 bg-surface-900/80 backdrop-blur-sm shrink-0">
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => createSession()} className="p-1.5 rounded-lg text-slate-400 hover:text-white hover:bg-surface-800 transition-colors" title="New chat">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
            </svg>
          </button>
          <div className="flex items-center gap-2">
            <div className={`w-2 h-2 rounded-full ${messages.some(m => m.role === 'assistant' && m.isStreaming) ? 'bg-yellow-400 animate-pulse' : isConnected ? 'bg-green-400' : 'bg-red-400'}`} />
            <span className="text-xs text-slate-500 hidden sm:inline">
              {messages.some(m => m.role === 'assistant' && m.isStreaming) ? 'Thinking...' : isConnected ? 'Ready' : 'Connecting...'}
            </span>
          </div>
        </div>
      </header>
      ...
      <div className="border-t border-surface-800 bg-surface-900/80 backdrop-blur-sm p-3 shrink-0">
        <form onSubmit={handleSubmit} className="max-w-3xl mx-auto">
          <div className="relative">
            <textarea
              ...
              placeholder={messages.some(m => m.role === 'assistant' && m.isStreaming) ? 'Processing... (Esc to abort)' : 'Message π Agent...'}
            ...
            <div className="absolute right-2 bottom-2 flex items-center gap-1">
              {messages.some(m => m.role === 'assistant' && m.isStreaming) ? (
                <button type="button" onClick={abort} className="p-1.5 rounded-lg text-red-400 hover:bg-red-900/30" title="Abort">
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><rect x="6" y="6" width="12" height="12" rx="1" /></svg>
                </button>
              ) : (
                <button type="submit" disabled={!input.trim() || !isConnected} className="p-1.5 rounded-lg bg-accent text-white hover:bg-accent-dark disabled:opacity-30 disabled:cursor-not-allowed">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <p className="text-center text-[10px] text-slate-600 mt-2">π Agent runs locally on your Mac</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full text-center px-4">
      ...
      <p className="text-sm text-slate-500 max-w-md mb-8">
        Send a message to start a conversation. Use the sidebar to switch between projects.
      </p>
