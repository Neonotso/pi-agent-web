...
`npm start` uses `node --import tsx server/index.ts`. Do not run `node server/index.ts` directly; the server source is TypeScript.
...
| `npm run serve` | Build frontend, then run production server |

See [SERVER_RUNBOOK.md](./SERVER_RUNBOOK.md) for the exact live tmux restart and verification commands.
...
| `{ type: "slash_command" }` | Handle web/Pi slash commands from chat |
